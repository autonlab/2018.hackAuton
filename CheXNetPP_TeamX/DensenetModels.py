...
class DenseNet121(nn.Module):
    ...
    def __init__(self, classCount, isTrained):
        ...
        self.densenet121.classifier = nn.BatchNorm1d(kernelCount) #nn.Linear(kernelCount,kernelCount)

        self.fc1 = nn.Linear(3+kernelCount,kernelCount)
        self.r1 = nn.ReLU()
        self.fc2 = nn.Linear(kernelCount,classCount)
        self.sig=nn.Sigmoid()

    def forward(self, x):
        x,y = torch.split(x, 150528, dim =1)

        x = x.contiguous().view(-1,3,224,224).contiguous()


        x = self.densenet121(x.float())

        xx = x.view(-1,self.kernelCount)
